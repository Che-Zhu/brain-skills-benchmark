...
1. **Kaniko / VersityGW build path** — For fresh builds, use the sandbox skill's \`k8s-kaniko-job\` flow. Verify \`kubectl\` namespace permissions, the DevBox VersityGW S3 endpoint (\`S3_ENDPOINT\` or compatible env), S3 secret source, and \`GITHUB_TOKEN\`/GHCR preflight before creating a Job. \`S3_ENDPOINT=http://127.0.0.1:1319\` is valid only inside the DevBox process; the separate kaniko Pod must receive a Pod-IP, Service, or otherwise cluster-reachable S3 endpoint such as the skill-resolved \`KANIKO_JOB_S3_ENDPOINT\`. Do **not** use BuildKit, \`buildctl\`, or \`k8s-buildkit-job\` for this benchmark.
