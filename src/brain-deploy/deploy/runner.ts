...
import {
  ...
  refreshDevboxPause,
  resumeDevbox,
} from "../devbox/client.js";
import {
  getDevboxArchiveAfterPauseTime,
  getDevboxDefaultImage,
} from "../devbox/config.js";
import type { DevboxInfo } from "../devbox/types.js";
import { DEPLOY_SKILL_NAME, getBrainSandboxSkillsGit } from "../load-env.js";
import { getGithubAccessToken } from "./github-token.js";

import { prepareDeployTaskArtifacts } from "./artifacts.js";
import {
...
function installSkillsCommand(): string {
  return [
    "set -euo pipefail",
    `workspace_dir=${shellQuote(DEPLOY_WORKSPACE_DIR)}`,
    `agent_skill_marker="$workspace_dir/.agents/skills/${DEPLOY_SKILL_NAME}/SKILL.md"`,
    `codex_skill_marker="$workspace_dir/.codex/skills/${DEPLOY_SKILL_NAME}/SKILL.md"`,
    'if [ ! -f "$agent_skill_marker" ] && [ ! -f "$codex_skill_marker" ]; then',
    "if command -v npx >/dev/null 2>&1; then",
    '  cd "$workspace_dir"',
    `  timeout 120 npx --yes skills add ${shellQuote(getBrainSandboxSkillsGit())} -y`,
    "else",
    "  printf 'ERROR: npx is required to install deploy skills\\n' >&2",
    "  exit 1",
    "fi",
    "fi",
    'if [ ! -f "$agent_skill_marker" ] && [ ! -f "$codex_skill_marker" ]; then',
    `  printf 'ERROR: ${DEPLOY_SKILL_NAME} skill not found after install\\n' >&2`,
    "  exit 1",
    "fi",
  ].join("\n");
}
